	...
	<form action="" method="get" class="form form-horizontal" id="form-article-add">
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<label><?php echo $specInfo['name']; ?></label>
				<input type="hidden" name="spec_id" value="<?php echo $specInfo['id']; ?>" >
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>规格列表：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="item" cols="" rows="" class="textarea" placeholder="规格列表 (一行一个)" datatype="*10-100" dragonfly="true" nullmsg="不能为空！"><?php echo $text; ?></textarea>
			</div>
		</div>
		<div class="row cl">
...
<script>
    function form_submit()
    {
        ...
        $.ajax({
            type: "GET",
            url: "<?php echo url('admin/Goods/specItemSave','',''); ?>"+'?'+$("#form-article-add").serialize(),
            data: {},
            dataType: "json",
