...
<div class="page-container">
    <form action="#" method="get">
        <div class="text-c">
            ...
            <span class="select-box inline">
                ...
                <option value="weixin" <?php if(\think\Request::instance()->get('oauth') == 'weixin'): ?>selected<?php endif; ?> >微信</option>
                <option value="qq" <?php if(\think\Request::instance()->get('oauth') == 'qq'): ?>selected<?php endif; ?> >qq</option>
                <option value="alipay" <?php if(\think\Request::instance()->get('oauth') == 'alipay'): ?>selected<?php endif; ?> >支付宝</option>
                </select>
            </span>&nbsp;&nbsp;&nbsp;
            <span class="select-box inline">
                <select class="select" name="status">
                <option value="">状态</option>
                <option value="1" <?php if(\think\Request::instance()->get('status') == '1'): ?>selected<?php endif; ?> >正常</option>
                    <option value="0" <?php if(\think\Request::instance()->get('status') == '0'): ?>selected<?php endif; ?> >禁止</option>
                </select>
            </span>&nbsp;&nbsp;&nbsp;
            <span class="select-box inline">
                <select class="select" name="search_type">
                <option value="">搜索类型</option>
                    <option value="1" <?php if(\think\Request::instance()->get('search_type') == '1'): ?>selected<?php endif; ?> >手机号</option>
                    <option value="2" <?php if(\think\Request::instance()->get('search_type') == '2'): ?>selected<?php endif; ?> >用户id</option>
                    <option value="3" <?php if(\think\Request::instance()->get('search_type') == '3'): ?>selected<?php endif; ?> >名称</option>
                </select>
            </span>&nbsp;&nbsp;&nbsp;
            <input type="text" class="input-text" style="width:250px" placeholder="搜索内容" id="" name="search_text" value="<?php echo \think\Request::instance()->get('search_text'); ?>">
    ...
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover table-responsive">
            ...
            <tr class="text-c">
                ...
                <td><?php echo $vo['oauth']; ?></td>
                <td><img src="<?php echo $vo['head_pic']; ?>"/></td>
                <td><?php echo $vo['nickname']; ?></td>
                <td><?php echo date("Y-m-d H:i:s",$vo['create_at']); ?></td>
                <td><?php echo date("Y-m-d H:i:s",$vo['last_login']); ?></td>
                <td><?php echo $vo['last_ip']; ?></td>
                <td><a class="badge <?php if($vo['status'] == '1'): ?>badge-success<?php else: ?>label-default<?php endif; ?> radius" href="javascript:;"><?php if($vo['status'] == '1'): ?>正常<?php else: ?>禁止<?php endif; ?></a></td>
    ...
    <div class="text-c"><?php echo $userList->render(); ?></div>
</div>
