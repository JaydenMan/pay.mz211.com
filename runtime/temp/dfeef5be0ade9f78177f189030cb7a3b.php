	...
	<form action="" method="get" class="form form-horizontal" id="form-article-add">
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<label><?php echo $goodTypeInfo['name']; ?></label>
				<input type="hidden" value="<?php echo $goodTypeInfo['id']; ?>" name="type_id">
			</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo !empty($info['order'])?$info['order'] : ''; ?>" placeholder="" name="order">
			</div>
		</div>
		<div class="row cl">
...
<script>
    function form_submit()
    {
        ...
        if(order.trim() == "" ) {
            layer.msg('请输入排序!',{icon:2,time:1000});
            return false;
        }
        $.ajax({
            type: "GET",
            url: "<?php echo url('admin/Goods/goodsTypeSpecSave','',''); ?>"+'?'+$("#form-article-add").serialize(),
            data: {},
            dataType: "json",
